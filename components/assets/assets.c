...
static const asset_info_t asset_info_testnet[] = {
    // These are not present in the asset registry data, or override what is there (as they are first).
...
// Get h/coded asset info (by hex id)
const asset_info_t* assets_get_info(const char* asset_id, const bool use_testnet_registry)
{
    assert(asset_id);

    const asset_info_t* assets = use_testnet_registry ? asset_info_testnet : asset_info;
    const size_t sizeof_asset_array = use_testnet_registry ? sizeof(asset_info_testnet) : sizeof(asset_info);

    const size_t nassets = sizeof_asset_array / sizeof(asset_info_t);
    for (const asset_info_t* passet = assets; passet < assets + nassets; ++passet)
    {
        assert(passet);
