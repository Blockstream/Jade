...
const color_t TFT_DARKGREEN   = { 255, 128, 255 };
const color_t TFT_DARKCYAN    = { 255, 128, 128 };
const color_t TFT_MAROON      = { 128, 255, 255 };
const color_t TFT_PURPLE      = { 128, 255, 128 };
const color_t TFT_OLIVE       = { 128, 128, 255 };
const color_t TFT_LIGHTGREY   = {  63,  63,  63 };
const color_t TFT_DARKGREY    = { 128, 128, 128 };
const color_t TFT_BLUE        = { 255, 255,   0 };
const color_t TFT_GREEN       = { 255,   0, 255 };
const color_t TFT_CYAN        = { 255,   0,   0 };
const color_t TFT_RED         = {   3, 255, 255 };
const color_t TFT_MAGENTA     = {   3, 255,   0 };
const color_t TFT_YELLOW      = {   3,   3, 255 };
const color_t TFT_WHITE       = {   3,   3,   3 };
...
void TFT_setFont(uint8_t font, const char *font_file)
{
  ...
  if (font == FONT_7SEG) {
    ...
    cfont.color  = _fg;
	  ...
	  else if (font == BIG_FONT) cfont.font = tft_BigFont;
	  else if (font == SINCLAIR_M) cfont.font = tft_Sinclair_M;
	  else if (font == SINCLAIR_S) cfont.font = tft_Sinclair_S;
	  else if (font == RETRO_8X16) cfont.font = tft_Retro8x16;
...
// This is faster than printing with setclipwin
void TFT_print_in_area(const char *st, int x, int y, dispWin_t areaWin) {
	int stl, i, tmpw, tmph, fh;
	uint8_t ch;
	...
	if ((x >= LASTX) && (x < LASTY)) x = TFT_X + (x-LASTX);
	else if (x > CENTER) x += areaWin.x1;

	if (y >= LASTY) y = TFT_Y + (y-LASTY);
	else if (y > CENTER) y += areaWin.y1;
	...
	if (x == RIGHT) x = areaWin.x2 - tmpw;
	else if (x == CENTER) x = ((areaWin.x2 - areaWin.x1 - tmpw) / 2) + areaWin.x1;

	if (y == BOTTOM) y = areaWin.y2 - fh;
	else if (y==CENTER) y = ((areaWin.y2 - areaWin.y1 - fh) / 2) + areaWin.y1;

	if (x < areaWin.x1) x = areaWin.x1;
	if (y < areaWin.y1) y = areaWin.y1;
	if ((x > areaWin.x2) || (y > areaWin.y2)) return;
	...
	if ((TFT_Y + tmph - 1) > areaWin.y2) return;
	...
	for (i=0; i<stl; i++) {
		...
		if (ch == 0x0D) { // === '\r', erase to eol ====
			if ((!font_transparent) && (font_rotate==0)) _fillRect(TFT_X, TFT_Y,  areaWin.x2+1-TFT_X, tmph, _bg);
		}

		else if (ch == 0x0A) { // ==== '\n', new line ====
			if (cfont.bitmap == 1) {
				TFT_Y += tmph + font_line_space;
				if (TFT_Y > (areaWin.y2-tmph)) break;
				TFT_X = areaWin.x1;
			}
		}
...
static inline bool get_pixel(uint16_t x, uint16_t y, uint16_t width, const Icon *icon) {
    uint32_t val = ((uint32_t) width) * y + x;

    uint32_t elem = val / 32;
    uint32_t bit = val % 32;
...
int TFT_icon(const Icon *imgbuf, int x, int y, color_t color, dispWin_t area) {
    ...
    uint16_t height = imgbuf->height;

    uint16_t draw_width = min(width, area.x2 - area.x1);
    uint16_t draw_height = min(height, area.y2 - area.y1);
	...
	if (x == RIGHT) {
        ...
        start_x = (width - draw_width) / 2; // in case we are clipping it make sure it's centered
    } else {
        x = x + area.x1;
    }
...
#define _16_TO_R(x) (0xFF - ((x >> 8) & 0b11111000))
#define _16_TO_G(x) (0xFF - ((x >> 3) & 0b11111100))
#define _16_TO_B(x) (0xFF - ((x << 3) & 0b11111111))
...
int TFT_picture(const Picture *imgbuf, int x, int y, dispWin_t area) {
    ...
    uint32_t draw_width = min(width, area.x2 - area.x1);
    uint32_t draw_height = min(height, area.y2 - area.y1);
    ...
    color_t *color_line = heap_caps_malloc(draw_width*3, MALLOC_CAP_DMA);
    assert(color_line);
    disp_select();

    for (uint32_t loop_y = 0; loop_y < draw_height; loop_y++) {
        for (uint32_t loop_x = 0; loop_x < draw_width; loop_x++) {
            uint32_t index = width * loop_y + loop_x;

            if (imgbuf->bytes_per_pixel == 1) { // Grayscale
                color_line[loop_x].r = 0xFF - imgbuf->data_8[index];
                color_line[loop_x].g = 0xFF - imgbuf->data_8[index];
                color_line[loop_x].b = 0xFF - imgbuf->data_8[index];
            } else if (imgbuf->bytes_per_pixel == 2) { // RGB565
                color_line[loop_x].r = _16_TO_R(imgbuf->data[index]);
                color_line[loop_x].g = _16_TO_G(imgbuf->data[index]);
                color_line[loop_x].b = _16_TO_B(imgbuf->data[index]);
            } else { // RGB
                color_line[loop_x].r = 0xFF - imgbuf->data_8[index * 3];
                color_line[loop_x].g = 0xFF - imgbuf->data_8[index * 3 + 1];
                color_line[loop_x].b = 0xFF - imgbuf->data_8[index * 3 + 2];
            }
        }

        send_data(x, y + loop_y, x + draw_width, y + loop_y + 1, draw_width, color_line);
    }
    free(color_line);
    disp_deselect();
