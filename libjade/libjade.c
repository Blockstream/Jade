...
// OTA. Not supported since the running libjade code can't be replaced
static void unsupported_process(void* process, char* name)
{
    JADE_LOGE("%s not supported by Software Jade", name);
    jade_process_reject_message(process, CBOR_RPC_INTERNAL_ERROR, "Unsupported");
}

void ota_process(void* p) { unsupported_process(p, "ota"); }
void ota_delta_process(void* p) { unsupported_process(p, "ota_deltaa"); }
