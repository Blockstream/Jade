    ...
    *selected_word = NULL;
    ...
    while (true) {
        ...
        if (possible_words <= NUM_WORDS_SELECT) {
            ...
            const unsigned int word_num = word_index + 1;
            const int ret = snprintf(choose_word_title, sizeof(choose_word_title), "Select word %u", word_num);
    ...
    gui_activity_t* choose_word_activity, gui_view_node_t* label, gui_view_node_t* text_selection)
{
    ...
    const unsigned int word_num = word_index + 1;
    const int ret = snprintf(confirm_word_title, sizeof(confirm_word_title), "Confirm word %u", word_num);
    ...
    const wordlist_purpose_t purpose, const size_t nwords, char* output, const size_t output_len)
{
    ...
    while (word_index < nwords && !done_entering_words) {
        ...
        if (is_mnemonic) {
            ...
            const unsigned int word_num = word_index + 1;
            const int ret = snprintf(enter_word_title, sizeof(enter_word_title), "Insert word %u", word_num);
    ...
    const size_t nwords, const bool advanced_mode, char* output, const size_t output_len)
{
    ...
    while (word_index < nwords) {
        ...
        char title[24];
        const unsigned int word_num = word_index + 1;
        const int ret = snprintf(title, sizeof(title), "Word %u/%u", word_num, (unsigned int)nwords);
