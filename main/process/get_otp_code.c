...
void get_otp_code_process(void* process_ptr)
{
    ...
    if (!otp_load_uri(otp_name, otp_uri, sizeof(otp_uri), &written) || !written) {
        jade_process_reject_message(process, CBOR_RPC_BAD_PARAMETERS, "Cannot find or load named otp record");
        goto cleanup;
    }
    ...
    if (!otp_uri_to_ctx(otp_uri, written, &otp_ctx)) {
        ...
        jade_process_reject_message(process, CBOR_RPC_BAD_PARAMETERS, "Failed to parse otp record");
        goto cleanup;
    }
    ...
    if (!otp_get_auth_code(&otp_ctx, token, sizeof(token))) {
        jade_process_reject_message(process, CBOR_RPC_INTERNAL_ERROR, "Failed to calculate otp token");
        goto cleanup;
    }
    ...
    if (!display_otp_screen(&otp_ctx, value, token, sizeof(token), confirm_only, auto_update)) {
        ...
        jade_process_reject_message(process, CBOR_RPC_USER_CANCELLED, "User declined OTP code");
        goto cleanup;
    }
