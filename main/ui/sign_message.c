...
void make_sign_message_activity(gui_activity_t** activity_ptr, const char* message_hash_hex, const char* path_as_str)
{
    ...
    char hash_str[128];
    const int ret = snprintf(hash_str, sizeof(hash_str), "} %s {", message_hash_hex);
    JADE_ASSERT(ret > 0 && ret < sizeof(hash_str));

    gui_view_node_t* text_hash;
    gui_make_text(&text_hash, hash_str, TFT_WHITE);
